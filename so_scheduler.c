...
int check_scheduler(int check)
{
	...
	if (scheduler.runningThread->status == WAITING) {
		...
		// place in waiting queue and schedule next thread from queue
 ...
 */
int check_signaled_threads(int check)
{
 ...
 */
int check_new_thread(int check)
{
 ...
 */
void free_waiting_threads(int arg)
{
...
int so_init(unsigned int time_quantum, unsigned int io)
{
	...
	if (time_quantum == 0 || io > SO_MAX_NUM_EVENTS || schedulerInit)
		return -1;
...
int so_signal(unsigned int io)
{
	...
	while (ioQueue->size) {
		...
		// update status and add in ready queue
		thread->status = READY;
		pq_add(scheduler.readyPq, thread);
		nrWokeThreads++;
	}
	...
	if (nrWokeThreads) {
		...
		// block current thread if preempted
		int preempted = check_signaled_threads(1);
...
void so_exec(void)
{
	...
	// check if time is up
	int preempted = check_scheduler(1);
